...
def decompose_pattern(
    k, n, pattern: Matrix[bool], bk: int, bn: int
) -> Tuple[Matrix[int], List[Matrix[bool]]]:
    Bk, Bn = k // bk, n // bn
    patterns = []
    x = 0
    ...
    for Bni in range(Bn):
        for Bki in range(Bk):
            if Bni + 1 == Bn and n_overhead > 0 and Bki + 1 == Bk and k_overhead > 0:
                ...
                block = pattern[
                    (Bki * bk) : ((Bki + 1) * bk), (Bni * bn) : ((Bni + 1) * bn)
                ]
...
class MatMul:
    def __init__(
        ...
        **kwargs,  # Accept and ignore args which don't belong
        ...
        pypspamm.architecture.Generator = pypspamm.architecture.get_class(
            "pypspamm.codegen.architectures." + arch + ".generator"
        ).Generator
        pypspamm.architecture.operands = pypspamm.architecture.get_class(
            "pypspamm.codegen.architectures." + arch + ".operands"
        )
        pypspamm.architecture.blocksize = pypspamm.architecture.get_class(
            "pypspamm.codegen.architectures." + arch + ".blocksize"
        ).Default

        self.generator = pypspamm.architecture.Generator(self.precision)
        ...
        (
            ...
            self.beta_reg,
            self.loop_regs,
            self.additional_regs,
            self.mask_regs,
            self.prefetch_reg,
        ) = self.generator.make_reg_blocks(
            self.bm,
            self.bn,
            self.bk,
            self.v_size,
        ...
        self.alpha_bcst_reg, self.beta_bcst_reg = (
            self.starting_regs[3],
            self.starting_regs[4],
        )

        if lda == 0:
            blocks, patterns, mtx_overhead = decompose_pattern(
                self.m, self.k, apattern, self.bm, self.bk
            )
            self.A = BlockCursor(
                "A",
                self.starting_regs[0],
                self.m,
                self.k,
                self.lda,
                self.bm,
        ...
        if ldb == 0:
            blocks, patterns, mtx_overhead = decompose_pattern(
                self.k, self.n, bpattern, self.bk, self.bn
            )
            self.B = BlockCursor(
                "B",
                self.starting_regs[1],
                self.k,
                self.n,
                self.ldb,
                self.bk,
    ...
    def microkernel(self, asm, Bmi, Bni, unroll, A_ptr, B_ptr, C_ptr, C_pf_ptr):
        ...
        regs = Matrix(
            [
                [
                    VirtualRegister(self.C_regs[0, 0].typeinfo, self.C_pool)
                    for _ in range(self.C_regs.shape[1])
                ]
                for _ in range(self.C_regs.shape[0])
            ]
        )
        ...
        if self.alpha in [-1.0, 1.0] and self.beta != 0.0:
            asm.add(
                self.generator.move_register_block(
                    self.C,
                    C_ptr_in,
                    to_C,
                    regs,
                ...
                for ic in range(regs.shape[1]):
                    for ir in range(regs.shape[0]):
                        pred_m = (
                            None
                            if not self.masks
                            else self.generator.pred_n_trues(
                                self.bm - ir * self.v_size, self.v_size, "m"
                            )
                        )
                        asm.add(
                            mul(
        ...
        def kernelK(asm, Bki):
            if unroll:
                ...
                if BmEnd and m_overhead > 0 and not self.unroll_m:
                    ...
                    A_ptr_in = A_ptr
                to_A = (
                    Coords(right=Bki, down=Bmi, absolute=True)
                    if self.unroll_m
                    else Coords(right=Bki)
                )
                ...
                if BnEnd and n_overhead > 0 and not self.unroll_n:
                    ...
                    B_ptr_in = B_ptr
                to_B = (
                    Coords(right=Bni, down=Bki, absolute=True)
                    if self.unroll_n
                    else Coords(down=Bki)
                )
                keep = (
                    not self.unroll_n or self.B.has_nonzero_block(B_ptr_in, to_B)
                ) and (not self.unroll_m or self.A.has_nonzero_block(A_ptr_in, to_A))
            else:
            ...
            if keep:
                ...
                asm.add(
                    self.generator.make_microkernel(
                        self.A,
                        self.B,
                        A_ptr_in,
                        B_ptr_in,
        ...
        if self.alpha not in [-1.0, 1.0]:
            ...
            if self.use_bcst:
                store_block.add(
                    bcst(self.alpha_bcst_reg, self.alpha_reg[1], "Broadcast alpha")
                )
    ...
    def blockloop(self, asm, A_ptr, B_ptr, C_ptr, C_pf_ptr):
        ...
        def outerLoop(asm, i):
            ...
            if self.C_pf:
                asm.add(
                    self.C_pf.move(C_pf_ptr, Coords(**{dirC2: 1, dirC: moveLength}))[0]
                )
    ...
    def loopwrap(
        ...
        if unroll:
            ...
            def makeLoop(until):
                ...
                return loop(loopreg, until, unroll=loopunroll, overlap=overlap).body(
                    loopblock
                )
    ...
    def make(self):
        ...
        asm.add(
            self.generator.make_argument_load(self.starting_regs, self.C_pf is not None)
        )
        ...
        asm.add(
            block(
                ...
                self.generator.init_mask(
                    self.m, self.bm, self.v_size, self.loop_regs[0], self.mask_regs
                ),
            )
        )
