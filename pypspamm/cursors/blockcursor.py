...
from pypspamm.cursors.coords import Coords
from pypspamm.cursors.matrix import Matrix
...
class BlockCursor(Cursor):
    ...
    def offset(self, src_loc: CursorLocation, dest_loc: CursorLocation) -> int:
    ...
    def move(
        self, src_loc: CursorLocation, dest_block: Coords
    ...
    def look(
        self, src_loc: CursorLocation, dest_block: Coords, dest_cell: Coords
    ...
    def get_block(
        self, src: CursorLocation = None, dest_block: Coords = None
    ) -> BlockInfo:
    ...
    def has_nonzero_cell(
        ...
        dest_cell += Coords(dest_block.down * self.br, dest_block.right * self.bc, True)
        return (
            self.offsets.shape[0] > dest_cell.down
            and self.offsets.shape[1] > dest_cell.right
            and self.offsets[dest_cell.down, dest_cell.right] != -1
        )
    ...
    def start_location(
        self, dest_block: Coords = Coords(absolute=True)
    ) -> CursorLocation:
            ...
            for bri in range(br):
                if pat[bri, bci]:
                    return CursorLocation(
                        dest_block, Coords(down=bri, right=bci, absolute=False)
                    )

        raise Exception(
            f"Block {dest_block} has no starting location because it is empty!"
        )
...
def sparse_mask(
    ...
    if not has_mask:
        assert (
            A_br % v_size == 0
        )  # bm must tile m exactly for now in non-mask-supporting ISAs
