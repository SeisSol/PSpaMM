...
"""
    ...
    def make_reg_blocks(
        ...
        B_regs = Matrix(
            [[v(vm * bk + bn * r + c, prec) for c in range(bn)] for r in range(vk)]
        )
        C_regs = Matrix(
            [[v(32 - vm * bn + vm * c + r, prec) for c in range(bn)] for r in range(vm)]
        )
            ...
            for ir in range(rows):
                if (mask is None) or (mask[ir, ic]):
                    ...
                    if prefetching:
                        ...
                        pfaddr = None
                    locations += [
                        self.LoadStoreLocation(addr, registers[ir, ic], comment, pfaddr)
                    ]

        return self.fuse_loadstore_block(
            locations, store, cursor.name, block_offset, additional_regs
        )

    def fuse_loadstore_block(
        self, locations, store, name, block_offset, additional_regs
    ):
        offsets = list(
            sorted([(location.addr.disp, location) for location in locations])
        )
        ...
        for _, location in offsets:
            ...
            if location.pfaddr is not None:
                if location.pfaddr.disp - curpf >= 32768:
                    asm.add(
                        add(
                            location.pfaddr.disp,
                            additional_regs[2],
                            "increment the prefetch register",
                            location.pfaddr.base,
                        )
                    )
