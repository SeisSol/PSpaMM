...
from pypspamm.codegen.microkernel import LoadedOnce, cells, contribution
            ...
            for ir in range(rows):
                if (mask is None) or (mask[ir, ic]):
                    ...
                    if all(has_nonzero):
                        ...
                        asm.add(
                            mov(
                                self.make_expand_predicate(all_coords),
                                additional_regs[0],
                                False,
                            )
                        )
