...
class Loop(Block):
    ...
    def contents(self):
        ...
        if true_final_val == self.unroll:
            ...
            allcode += (
                [
                    mov(-true_final_val, self.iteration_var, vector=False),
                    label(self.label),
                ]
                + body
                + [
                    add(self.unroll, self.iteration_var),
    ...
    def normalize(self):
        yield loop(
            self.iteration_var, self.final_val, self.unroll, self.may_overlap
        ).body(
            *[
                substmt
                for stmt in self.body_contents.contents
                for substmt in stmt.normalize()
            ]
        )
...
class Skip(Block):
    ...
    def __init__(self, skipreg: Register) -> None:
    ...
    def contents(self):
        ...
        return (
            [jump(self.label, self.skipreg, backwards=True)]
            + body
            + [label(self.label)]
        )
