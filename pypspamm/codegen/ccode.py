...
def make_cfunc(
    ...
    body: Block,
    flop: int,
    starting_regs: List[Register],
    precision: Precision,
) -> str:
    Printer_class = pypspamm.architecture.get_class(
        "pypspamm.codegen.architectures."
        + pypspamm.architecture.arch
        + ".inlineprinter"
    ).InlinePrinter
    ...
    regs = set(
        f'"{reg.clobbered}"'
        for reg in analyzer.clobbered_registers
        if reg.clobbered is not None
    )
    regs.add('"memory"')
    regs.add('"cc"')
