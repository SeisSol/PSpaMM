...
def add(src: Union[Operand, int], dest: Register, comment: str = None, additional: Register = None):
    stmt = AddStmt()
    stmt.src = src if isinstance(src, Operand) else pypspamm.architecture.operands.c(src)
    stmt.dest = dest
...
def cmp(lhs: Union[Operand, int], rhs: Union[Operand, int]):
    stmt = CmpStmt()
    stmt.lhs = lhs if isinstance(lhs, Operand) else pypspamm.architecture.operands.c(lhs)
    stmt.rhs = rhs if isinstance(rhs, Operand) else pypspamm.architecture.operands.c(rhs)
    return stmt
...
def data(value: Union[Operand, int], asmType=AsmType.i64):
    stmt = DataStmt()
    stmt.value = value if isinstance(value, Operand) else pypspamm.architecture.operands.c(value)
...
def rvsetvl(actual: Register, requested: Union[Register, int]):
    stmt = RVSetVLStmt()
    stmt.actual = actual
    stmt.requested = requested if isinstance(requested, Operand) else pypspamm.architecture.operands.c(requested)
    return stmt
