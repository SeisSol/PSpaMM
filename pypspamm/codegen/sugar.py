...
def add(
    ...
    stmt = AddStmt()
    stmt.src = (
        src if isinstance(src, Operand) else pypspamm.architecture.operands.c(src)
    )
    stmt.dest = dest
...
def label(name: str):
    stmt = LabelStmt()
    stmt.label = pypspamm.architecture.operands.l(name)
    return stmt
...
def cmp(lhs: Union[Operand, int], rhs: Union[Operand, int]):
    stmt = CmpStmt()
    stmt.lhs = (
        lhs if isinstance(lhs, Operand) else pypspamm.architecture.operands.c(lhs)
    )
    stmt.rhs = (
        rhs if isinstance(rhs, Operand) else pypspamm.architecture.operands.c(rhs)
    )
    return stmt
...
def jump(label: str, cmpreg=None, backwards=True):
    stmt = JumpStmt()
    stmt.destination = pypspamm.architecture.operands.l(label)
    stmt.cmpreg = cmpreg
    return stmt
...
def ld(
    ...
    pred: Register = None,
    is_B: bool = False,
    scalar_offs: bool = False,
    add_reg: AsmType.i64 = None,
    sub128: bool = False,
    expand=None,
    dest3: Operand = None,
    dest4: Operand = None,
...
def data(value: Union[Operand, int], asmType=AsmType.i64):
    stmt = DataStmt()
    stmt.value = (
        value if isinstance(value, Operand) else pypspamm.architecture.operands.c(value)
    )
...
def rvsetvl(actual: Register, requested: Union[Register, int]):
    ...
    stmt.requested = (
        requested
        if isinstance(requested, Operand)
        else pypspamm.architecture.operands.c(requested)
    )
    return stmt
